...
export default function SettingsPage() {
  ...
  async function test(kind) {
    ...
    try {
      ...
      const ok = kind === 'pdc'
        ? (s.authenticated ? 'connected' : s.reachable ? 'reachable (check credentials)' : 'offline')
        : (s.reachable ? `connected${s.dag_count != null ? ` · ${s.dag_count} DAGs` : ''}` : 'offline')
